...
// My Account Route
router.get('/:username', checkUser, (req, res) => {
    res.render('account', {
...
router.get('/:username/edit_profile', checkUser, (req, res) => {
...
router.post('/:username/edit_profile', checkUser, async (req, res) => {
    ...
    pool.query( sqlStatement, sqlParameter, (err) => {
        ...
        res.status(200).redirect(`/account/${username}`)
    })
})
